...
function ValidateReviewTitle(txt) {
    ...
    if (regAdd.test(txt) === false) {
        document.getElementById("err-reviewTitle").innerHTML = "<span class='warning'>Заголовок должен содержать от 5 до 100 символов!</span>";
...
function ValidateReviewContent(txt) {
    ...
    if (regAdd.test(txt) === false) {
        document.getElementById("err-reviewContent").innerHTML = "<span class='warning'>Отзыв должен содержать от 10 до 500 символов!</span>";
...
function DisplayErrors() {
    ...
    if (IsValidReviewStar === false) {
        document.getElementById("err-reviewStar").innerHTML = "<br /><br /><span class='warning'>Пожалуйста, выберите оценку книги!</span>";
...
function AddReview(bookId) {
    ...
    if (IsValidReviewStar === true && isValidReviewTitle === true && isValidReviewContent === true) {
        ...
        $.ajax({
            ...
            error: function (data) {
                document.getElementById("err-addReview").innerHTML = "<br /><br /><span class='warning'>Что-то пошло не так!</span>";
