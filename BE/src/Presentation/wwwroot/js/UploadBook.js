$(document).ready(function () {
    ...
    $('#extendReason').bind('input propertychange', function (e) {
        ...
        document.getElementById('err-extendReason').innerText = isExtendReasonValid(e.target) || e.target.value == "" ? "": "Причина продления должна содержать минимум 5 символов";
    });
    ...
    $('#extendSubmitButton').on('click', function () {
        if (isExtendReasonValid(document.getElementById('extendReason')))
        {
            ...
            $.ajax({
                ...
                error: function () {
                    document.getElementById('err-extendReason').innerText = "Плохой запрос. Введите данные корректно";
                }
            });
        }
    });
...
function validateTitle(txt) {
    ...
    if (regAdd.test(txt) === false) {
        document.getElementById("err-title").innerHTML = "<span class='warning'>Неверное название книги!</span>";
...
function validateFirstName(firstN) {
    let regAdd = /^а-яА-Я[a-zA-Z]{2,15}$/
    if (regAdd.test(firstN) === false) {
        document.getElementById("err-firstName").innerHTML = "<span class='warning'>Неверное имя!</span>";
...
function validateLastName(lastN) {
    let regAdd = /^[а-яА-Яa-zA-Z]{2,15}$/
    if (regAdd.test(lastN) === false) {
        document.getElementById("err-lastName").innerHTML = "<span class='warning'>Неверная фамилия!</span>";
...
function validateLanguage(language) {
    if (!language) {
        document.getElementById("err-language").innerHTML = "<span class='warning'>Выберите язык книги!</span>";
...
function ValidateGenre(genre) {
    if (!genre) {
        document.getElementById("err-genre").innerHTML = "<span class='warning'>Выберите жанры книги!</span>";
...
function ValidateFullName(author) {
    let regAdd = /^[А-ЯA-Z][а-яa-z]{1,15}(?: [А-ЯA-Z][а-яa-z]{1,15})$/
    if (regAdd.test(author) === false) {
        document.getElementById("err-addedAuthor").innerHTML = "<span class='warning'>Неверное имя автора</span>";
...
function ValidateEditedAuthorName(author, id) {
    let regAdd = /^[А-ЯA-Z][а-яa-z]{1,15}(?: [А-ЯA-Z][а-яa-z]{1,15})$/
    if (regAdd.test(author) === false) {
        document.getElementById("err-addedAuthor"+"-"+id).innerHTML = "<span class='warning'>Неверное имя автора</span>";
...
function ValidateExistingAuthor(existingAuthor) {
    if (!existingAuthor) {
        document.getElementById("err-existingAuthor").innerHTML = "<span class='warning'>Пожалуйста, укажите автора книги!</span>";
...
function AddNewAuthorToggle() {
    ...
    if (input.style.display === "none") {
        ...
        addButton.textContent = "Существующие";
        ...
        isAddedAuthorValid = false;
        if (($('#selectAuthor option').filter(':selected').text()) != "Выбрать автора") {
            isExistingAuthorValid = true;
        }
    }
}
...
function ValidateBookCover(oInput) {
    if (oInput.type == "file") {
        ...
        if (sFileName.length > 0) {
            ...
            if (!blnValid) {
                document.getElementById("err-cover").innerHTML = "<span class='warning'>Пожалуйста, выберите корректное изображение!</span>";
    ...
    if (document.getElementById('imageFile').files[0].size > 1024000) {
        document.getElementById("err-cover").innerHTML = "<span class='warning'>Файл не должен весить больше 1MB!</span>";
        document.getElementById("err-cover").style.color = "#FF0000";
        document.getElementById("imageFile").style.border = "none";
        oInput.value = "";
        isCoverValid = false;
...
async function ValidateUserAvatar(oInput) {
    ...
    if (oInput.type == "file") {
        ...
        if (sFileName.length > 0) {
            ...
            if (!blnValid) {
                document.getElementById("err-avatar").innerHTML = "<span class='warning' style='color:red'>Пожалуйста, выберите корректное изображение!</span>";
    ...
    if (document.getElementById('file').files[0].size > 1024000) {
        document.getElementById("err-avatar").innerHTML = "<span class='warning' style='color:red'>Файл не должен весить больше 1MB!</span>";
        document.getElementById("err-avatar").style.color = "#FF0000";
        document.getElementById("file").style.border = "none";
        oInput.value = "";
    } else {
       ...
       $.ajax({
            ...
            success: function (data) {
                document.getElementById("err-avatar").innerHTML = "<span class='success' style='color:green'>Ваш аватар был успешно загружен!</span>";
            },
            error: function (data) {
                document.getElementById("err-avatar").innerHTML = "<span class='warning' style='color:red'>Что-то пошло не так</span>";
            }
        });
    }
}
...
function ValidateDate(date) {
    if (formatDate(Date()) < date) {
        document.getElementById("err-publish").innerHTML = "<span class='warning'>Пожалуйста, укажите дату публикации</span>";
...
function DisplayFullNameError() {
    if (isEditedAuthorValid == false) {
        alert("Пожалуйста, укажите корректное имя автора!")
    }
}

function DisplayAuthorErrors() {
    if (isFirstNameValid === false && isLastNameValid == true) {
        alert("Пожалуйста, укажите корректное имя!")
        document.getElementById("err-firstName").innerHTML = "<span class='warning'>Некорректное имя!</span>";
        document.getElementById("err-firstName").style.color = "#FF0000";
        document.getElementById("firstName").style.border = "none"
    } else {
        alert("Пожалуйста, укажите корректную фамилию!")
        document.getElementById("err-lastName").innerHTML = "<span class='warning'>Некорректное фамилия!</span>";
        document.getElementById("err-lastName").style.color = "#FF0000";
        document.getElementById("lastName").style.border = "none";
    }
}
...
function DisplayErrors() {
    if (isTitleValid === false) {
        document.getElementById("err-title").innerHTML = "<span class='warning'>Некорректное название книги!</span>";
        document.getElementById("err-title").style.color = "#FF0000";
        document.getElementById("title").style.border = "none"
    }
    if (isLanguageValid === false) {
        document.getElementById("err-language").innerHTML = "<span class='warning'>Укажите язык книги!</span>";
        document.getElementById("err-language").style.color = "#FF0000";
        document.getElementById("selectLanguage").style.border = "none";
    }
    if (isGenreValid === false) {
        document.getElementById("err-genre").innerHTML = "<span class='warning'>Выберите жанр книги!</span>";
        document.getElementById("err-genre").style.color = "#FF0000";
        document.getElementById("selectGenre").style.border = "none";
    }
    if (isAddedAuthorValid === false && isExistingAuthorValid === false) {
        if (document.getElementById("insertNewAuthor").style.display === "none") {
            ...
            document.getElementById("err-addedAuthor").innerHTML = "<span class='warning'>Некорректное имя автора</span>";
            document.getElementById("err-addedAuthor").style.color = "#FF0000";
            document.getElementById("insertNewAuthor").style.border = "none";
        }
    }
    if (isDateValid === false) {
        document.getElementById("err-publish").innerHTML = "<span class='warning'>Пожалуйста, укажите корректную дату публикации</span>";
        document.getElementById("err-publish").style.color = "#FF0000";
        document.getElementById("setDate").style.border = "none";
    }
}
...
function ShowSuccessMessage() {
    ...
    $('<br /><div class="alert alert-success" role="alert" style="display: inline-block">Ваша книга была успешно добавлена!</div >').appendTo("#MessageBoxArea");
    isTitleValid = false;
    isDateValid = false;
    isAddedAuthorValid = false;
}
...
function ShowAuthorSuccessMessage() {
    ...
    $(".modal-body input").val("");
    document.getElementById("MessageBoxArea").innerHTML = "";
    $('<br /><div class="alert alert-success" role="alert" style="display: inline-block">Автор был успешно добавлен!</div >').appendTo("#MessageBoxArea");
    isTitleValid = false;
    isDateValid = false;
    isAddedAuthorValid = false;
}
...
function ShowAuthorAlreadyExistsMessage() {
    ...
    $(".modal-body input").val("");
    document.getElementById("MessageBoxArea").innerHTML = "";
    $('<br /><div class="alert alert-success" role="alert" style="display: inline-block">Такой автор уже существует.</div >').appendTo("#MessageBoxArea");
    isTitleValid = false;
    isDateValid = false;
    isAddedAuthorValid = false;
}
...
function CheckAuthorFields() {
    ...
    $(".modal-body input").val("");
    document.getElementById("MessageBoxArea").innerHTML = "";
    $('<br /><div class="alert alert-danger" role="alert" style="display: inline-block">Проверьте поля и попробуйте еще раз.</div >').appendTo("#MessageBoxArea");
    isTitleValid = false;
    isDateValid = false;
    isAddedAuthorValid = false;
}
...
function ShowErrorMessage() {
    ...
    $(".modal-body input").val("");
    document.getElementById("MessageBoxArea").innerHTML = "";
    $('<br /><div class="alert alert-danger" role="alert" style="display: inline-block">Пожалуйста, проверьте поля и попробуйте еще раз.</div >').appendTo("#MessageBoxArea")
    isTitleValid = false;
    isDateValid = false;
    isAddedAuthorValid = false;
}
