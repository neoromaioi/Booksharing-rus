...
function AddNewAuthorWishBookToggle() {
    ...
    if (input.style.display === "none") {
        ...
        isAddedAuthorValid = false;
        if (($('#selectAuthorWishBook option').filter(':selected').text()) != "Выбрать автора") {
...
function ValidateExistingWishBookAuthor(existingAuthor) {
    if (!existingAuthor) {
        document.getElementById("err-existingWishBookAuthor").innerHTML = "<span class='warning'>Пожалуйста, укажите автора!</span>";
...
function validateWishBookTitle(txt) {
    ...
    if (regAdd.test(txt) === false) {
        document.getElementById("err-wishBookTitle").innerHTML = "<span class='warning'>Некорректное название книги!</span>";
...
function ValidateWishBookFullName(author) {
    let regAdd = /^[а-яА-Яa-zA-Z]{2,15}(?: [а-яА-Яa-zA-Z]{2,15})$/
    if (regAdd.test(author) === false) {
        document.getElementById("err-addedWishBookAuthor").innerHTML = "<span class='warning'>Некорректное имя автора!</span>";
...
function DisplayWishBookErrors() {
    ...
    if (isAddedAuthorValid === false && isExistingAuthorValid === false) {
        if (document.getElementById("insertNewAuthorWishBook").style.display === "none") {
            document.getElementById("err-existingWishBookAuthor").innerHTML = "<span class='warning'>Пожалуйста, укажите автора книги!</span>";
            document.getElementById("err-existingWishBookAuthor").style.color = "#FF0000";
            document.getElementById("selectAuthorWishBook").style.border = "none";
        } else {
            document.getElementById("err-addedWishBookAuthor").innerHTML = "<span class='warning'>Некорректное имя автора!</span>";
...
$('#addWishBookModal').on('hidden.bs.modal', function () {
    ...
    document.getElementById('addWishBookAuthorButton').textContent = "Добавить автора";
...
function UploadThisBook(author, title) {
    ...
    if (!exist) {
        ...
        document.getElementById("addAuthorButton").textContent = "Существующие";
        ...
        document.getElementById("addAuthorButton").textContent = "Добавить автора";
        $('#addAuthorButton').attr('disabled', true);
        ...
        isAddedAuthorValid = false;
        if (($('#selectAuthor option').filter(':selected').text()) != "Укажите автора") {
            isExistingAuthorValid = true;
        }
