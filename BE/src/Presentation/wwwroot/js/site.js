        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                else
                {
                    ...
                    $("#assignBookModalContent").html("Вы успешно получили книгу.<br> Ваш срок обладания истекает:<br><b>" + data.date + "</b>");
                }
                wasAssigned = true;
    ...
    $('#addToQueueBtn').click(function () {
        if ($('#addToQueueBtn').hasClass('btn-disabled')) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $("#assignBookModalHeader").addClass("bg-success");
                    $("#assignBookModalContent").html("Книга была успешно добавлена в очередь. Ваш срок обладания стартует:<br><b>" + data.date + "</b>");
                    wasAssigned = true;
                    $("#assignBookModal").modal('show');
                }
            });
...
function assignAdmin(id, username) {
    ...
    $.ajax({
        ...
        success: function () {
            ...
            $("#userManageModalContent").html("Роль администратора была успешно назначена пользователю <b>" + username + "</b>");
           
        },
        error: function () {
            toggleModalErrorHeader("#userManageModalHeader", true)
            $("#userManageModalContent").html("Ошибка при назначении роли администратора пользователю <b>" + username + "</b>");
        }
    });
}
...
function removeAdmin(id, username) {
    ...
    $.ajax({
        ...
        success: function () {
            toggleModalErrorHeader("#userManageModalHeader", false)
            $("#userManageModalContent").html("Роль администратора была отнята у пользователя <b>" + username + "</b>");
          
        },
        error: function () {
            toggleModalErrorHeader("#userManageModalHeader", true)
            $("#userManageModalContent").html("Ошибка при лишении роли администратора пользователя <b>" + username + "</b>");
        }
    });
}
