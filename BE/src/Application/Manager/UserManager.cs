...
namespace Application.Manager
{
    public class UserManager : IUserManager
    {
        ...
        public AssignResultDTO AssignBookToCurrentUser(int bookId)
        {
            ...
            if (book == null)
            {
                result.Message = "Книги, которую вы хотите взять, не существует.";
            }
            else if (!book.IsAvailable)
            {
                result.Message = "Эта книга в данный момент не доступна.";
            }
            else if (!HasAssignmentsLeft(user))
            {
                result.Message = "Вы не можете иметь более чем " + AppSettings.Instance.MaxAllowedAssignments.ToString() + " активных взятых книг.";
            }
            else
            {
                result.Error = false;
